package starling.events
{
   ...
   internal class TouchMarker extends Sprite
   {
      ...
      public function TouchMarker()
      {
         ...
         while(_loc1_ < 2)
         {
            _loc2_ = new Image(this._texture);
            _loc2_.pivotX = this._texture.width / 2;
            _loc2_.pivotY = this._texture.height / 2;
      ...
      public function moveMarker(param1:Number, param2:Number, param3:Boolean = false) : void
      {
         if(param3)
         {
            this._center.x += param1 - this.realMarker.x;
            this._center.y += param2 - this.realMarker.y;
         }
         this.realMarker.x = param1;
         this.realMarker.y = param2;
         this.mockMarker.x = 2 * this._center.x - param1;
         this.mockMarker.y = 2 * this._center.y - param2;
      }
