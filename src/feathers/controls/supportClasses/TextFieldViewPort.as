package feathers.controls.supportClasses
{
   ...
   public class TextFieldViewPort extends FeathersControl implements IViewPort
   {
      ...
      override public function render(param1:Painter) : void
      {
         ...
         this.parent.getTransformationMatrix(this.stage,_loc4_);
         MatrixUtil.transformCoords(_loc4_,0,0,_loc5_);
         var _loc6_:Number = 1;
         if(_loc2_.supportHighResolutions)
         {
            _loc6_ = _loc2_.nativeStage.contentsScaleFactor;
         }
         var _loc7_:Number = _loc2_.contentScaleFactor / _loc6_;
         this._textFieldContainer.x = _loc3_.x + _loc5_.x * _loc7_;
         this._textFieldContainer.y = _loc3_.y + _loc5_.y * _loc7_;
         this._textFieldContainer.scaleX = matrixToScaleX(_loc4_) * _loc7_;
         this._textFieldContainer.scaleY = matrixToScaleY(_loc4_) * _loc7_;
         this._textFieldContainer.rotation = matrixToRotation(_loc4_) * 180 / Math.PI;
      ...
      override protected function draw() : void
      {
         ...
         var _loc3_:Boolean = this.isInvalid(INVALIDATION_FLAG_SCROLL);
         var _loc4_:Boolean = this.isInvalid(INVALIDATION_FLAG_STYLES);
         var _loc5_:Boolean = this.isInvalid(INVALIDATION_FLAG_STATE);
         if(_loc4_)
         {
         ...
         if(_loc1_ || _loc4_ || _loc5_)
         {
            ...
            else
            {
               ...
               {
                  this._textField.embedFonts = SystemUtil.isEmbeddedFont(this._currentTextFormat.font,this._currentTextFormat.bold,this._currentTextFormat.italic,FontType.EMBEDDED);
               }
               else
               {
                  this._textField.embedFonts = this._embedFonts;
            ...
            this._scrollStep = this._textField.getLineMetrics(0).height * _loc6_.contentScaleFactor;
         }
         var _loc7_:Number = this._explicitVisibleWidth;
         if(_loc7_ != _loc7_)
         {
            if(this.stage !== null)
            {
            ...
            {
               _loc7_ = _loc6_.stage.stageWidth;
            }
            if(_loc7_ < this._explicitMinVisibleWidth)
            {
               _loc7_ = this._explicitMinVisibleWidth;
            }
            else if(_loc7_ > this._maxVisibleWidth)
            {
               _loc7_ = this._maxVisibleWidth;
            }
         }
         this._textField.width = _loc7_ - this._paddingLeft - this._paddingRight;
         var _loc8_:Number = this._textField.height + this._paddingTop + this._paddingBottom;
         var _loc9_:Number = this._explicitVisibleHeight;
         if(_loc9_ != _loc9_)
         {
            _loc9_ = _loc8_;
            if(_loc9_ < this._explicitMinVisibleHeight)
            {
               _loc9_ = this._explicitMinVisibleHeight;
            }
            else if(_loc9_ > this._maxVisibleHeight)
            {
               _loc9_ = this._maxVisibleHeight;
            }
         }
         _loc2_ = this.saveMeasurements(_loc7_,_loc8_,_loc7_,_loc8_) || _loc2_;
         this._actualVisibleWidth = _loc7_;
         this._actualVisibleHeight = _loc9_;
         this._actualMinVisibleWidth = _loc7_;
         this._actualMinVisibleHeight = _loc9_;
         if(_loc2_ || _loc3_)
         {
            _loc10_ = this._textFieldContainer.scrollRect;
            if(!_loc10_)
            {
            ...
            _loc10_.x = this._horizontalScrollPosition;
            _loc10_.y = this._verticalScrollPosition;
            this._textFieldContainer.scrollRect = _loc10_;
         }
      }
      ...
      {
         this.dispatchEventWith(Event.TRIGGERED,false,param1.text);
      }
