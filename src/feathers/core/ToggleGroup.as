package feathers.core
{
   ...
   public class ToggleGroup extends EventDispatcher
   {
      ...
      public function set selectedIndex(param1:int) : void
      {
         ...
         var _loc3_:Boolean = this._selectedIndex != param1;
         this._selectedIndex = param1;
         var _loc4_:Boolean = this._ignoreChanges;
         this._ignoreChanges = true;
         var _loc5_:int = 0;
         while(_loc5_ < _loc2_)
         {
            _loc6_ = this._items[_loc5_];
            _loc6_.isSelected = _loc5_ == param1;
            _loc5_++;
         }
         this._ignoreChanges = _loc4_;
         if(_loc3_)
         {
      ...
      public function addItem(param1:IToggle) : void
      {
         ...
         param1.addEventListener(Event.CHANGE,this.item_changeHandler);
      ...
      public function removeItem(param1:IToggle) : void
      {
         ...
         this._items.removeAt(_loc2_);
         param1.removeEventListener(Event.CHANGE,this.item_changeHandler);
         if(param1 is IGroupedToggle)
         {
      ...
      {
         var _loc3_:IToggle = null;
         var _loc1_:int = int(this._items.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
            _loc3_ = this._items.shift();
            _loc3_.removeEventListener(Event.CHANGE,this.item_changeHandler);
            if(_loc3_ is IGroupedToggle)
            {
               IGroupedToggle(_loc3_).toggleGroup = null;
            }
            _loc2_++;
         }
         this.selectedIndex = -1;
      ...
      public function setItemIndex(param1:IToggle, param2:int) : void
      {
         ...
         this._items.removeAt(_loc3_);
         this._items.insertAt(param2,param1);
         if(this._selectedIndex >= 0)
         {
            if(this._selectedIndex == _loc3_)
            {
