package feathers.utils.textures
{
   ...
   public class TextureCache
   {
      ...
      public function dispose() : void
      {
         ...
         for each(_loc1_ in this._unretainedTextures)
         {
            _loc1_.dispose();
         }
      ...
      {
         if(Boolean(this._retainCounts) && param1 in this._retainCounts)
         {
      ...
      public function releaseTexture(param1:String) : void
      {
         ...
         var _loc2_:* = this._retainCounts[param1] as int;
         _loc2_--;
         if(_loc2_ == 0)
         {
            _loc3_ = Texture(this._retainedTextures[param1]);
            delete this._retainCounts[param1];
            delete this._retainedTextures[param1];
            this._unretainedTextures[param1] = _loc3_;
            this._unretainedKeys[this._unretainedKeys.length] = param1;
            if(this._unretainedKeys.length > this._maxUnretainedTextures)
            {
               this.trimCache();
         ...
         {
            this._retainCounts[param1] = _loc2_;
         }
      }
      ...
      protected function trimCache() : void
      {
         ...
         var _loc1_:* = int(this._unretainedKeys.length);
         var _loc2_:int = this._maxUnretainedTextures;
         while(_loc1_ > _loc2_)
         {
            _loc3_ = this._unretainedKeys.shift();
            _loc4_ = Texture(this._unretainedTextures[_loc3_]);
            _loc4_.dispose();
            delete this._unretainedTextures[_loc3_];
            _loc1_--;
         }
      }
   }
