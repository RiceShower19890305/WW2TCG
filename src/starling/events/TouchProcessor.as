package starling.events
{
   ...
   public class TouchProcessor
   {
      ...
      public function TouchProcessor(param1:Stage)
      {
         ...
         this._queue = new Vector.<TouchData>(0);
      ...
      public function advanceTime(param1:Number) : void
      {
         ...
         while(this._queue.length > 0 || _loc4_ == 0)
         {
            ...
            while(_loc2_ >= 0)
            {
               ...
               if(_loc3_.phase == TouchPhase.HOVER && !this.containsTouchWithID(sUpdatedTouches,_loc3_.id))
               {
            ...
            while(_loc2_ >= 0)
            {
               if(this._currentTouches[_loc2_].phase == TouchPhase.ENDED)
               {
      ...
      protected function processTouches(param1:Vector.<Touch>, param2:Boolean, param3:Boolean) : void
      {
         ...
         this._touchEvent.resetTo(TouchEvent.TOUCH,this._currentTouches,param2,param3);
         for each(_loc4_ in param1)
         {
            if(_loc4_.phase == TouchPhase.HOVER && Boolean(_loc4_.target))
            {
            ...
            if(_loc4_.phase == TouchPhase.HOVER || _loc4_.phase == TouchPhase.BEGAN)
            {
               sHelperPoint.setTo(_loc4_.globalX,_loc4_.globalY);
               if(this._occlusionTest != null && Boolean(this._occlusionTest(_loc4_.globalX,_loc4_.globalY)))
               {
                  _loc4_.target = null;
               }
               else
      ...
      private function checkForSystemGesture(param1:int, param2:String, param3:Number, param4:Number) : Boolean
      {
         ...
         if(!this._discardSystemGestures || param2 == TouchPhase.HOVER)
         {
            return false;
         }
         if(this._systemGestureTouchID == param1)
         {
            if(param2 == TouchPhase.ENDED)
            {
      ...
      public function enqueue(param1:int, param2:String, param3:Number, param4:Number, param5:Number = 1, param6:Number = 1, param7:Number = 1) : void
      {
         ...
         if(Boolean(this._ctrlDown) && Boolean(this._touchMarker) && param1 == 0)
         {
      ...
      {
         this._queue.unshift(TouchData.fromPool(param1,param2,param3,param4,param5,param6,param7));
      }
      ...
      public function cancelTouches() : void
      {
         ...
         if(this._currentTouches.length > 0)
         {
            ...
            {
               if(_loc1_.phase == TouchPhase.BEGAN || _loc1_.phase == TouchPhase.MOVED || _loc1_.phase == TouchPhase.STATIONARY)
               {
                  _loc1_.phase = TouchPhase.ENDED;
         ...
         {
            TouchData.toPool(this._queue.pop());
         }
      }
      
      private function createOrUpdateTouch(param1:TouchData) : Touch
      {
         var _loc2_:Touch = this.getCurrentTouch(param1.id);
         if(_loc2_ == null)
         {
         ...
         if(param1.phase == TouchPhase.BEGAN)
         {
      ...
      {
         var _loc2_:* = int(this._currentTouches.length - 1);
         while(_loc2_ >= 0)
         {
      ...
      private function onKey(param1:KeyboardEvent) : void
      {
         ...
         if(param1.keyCode == 17 || param1.keyCode == 15)
         {
            ...
            this._ctrlDown = param1.type == KeyboardEvent.KEY_DOWN;
            if(Boolean(this._touchMarker) && _loc2_ != this._ctrlDown)
            {
               ...
               {
                  this.queue_unshift(1,TouchPhase.ENDED,_loc4_.globalX,_loc4_.globalY);
               }
               else if(this._ctrlDown && Boolean(_loc3_))
               {
                  if(_loc3_.phase == TouchPhase.HOVER || _loc3_.phase == TouchPhase.ENDED)
                  {
                     this.queue_unshift(1,TouchPhase.HOVER,this._touchMarker.mockX,this._touchMarker.mockY);
                  }
                  else
                  {
                     this.queue_unshift(1,TouchPhase.BEGAN,this._touchMarker.mockX,this._touchMarker.mockY);
                  }
               }
            }
         }
         ...
         {
            this._shiftDown = param1.type == KeyboardEvent.KEY_DOWN;
         }
      }
