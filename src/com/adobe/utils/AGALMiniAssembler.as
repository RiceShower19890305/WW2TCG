package com.adobe.utils
{
   ...
   public class AGALMiniAssembler
   {
      ...
      private static function init() : void
      {
         ...
         OPMAP[TLD] = new OpCode(TLD,3,46,OP_VERT_ONLY | OP_SPECIAL_TEX);
         ...
         while(_loc10_ < _loc11_ && this._error == "")
         {
            ...
            else
            {
               ...
               else
               {
                  ...
                  _loc18_ = _loc12_.match(/vc\[([vofi][acostdip]?[d]?)(\d*)?((\.[xyzw])?(\+\d{1,3})?)?\](\.[xyzw]{1,4})?|([vofi][acostdip]?[d]?)(\d*)?(\.[xyzw]{1,4})?/gi);
                  ...
                  while(_loc22_ < _loc21_)
                  {
                     ...
                     _loc25_ = _loc18_[_loc22_].match(/^\b[A-Za-z]{1,3}/ig);
                     if(!_loc25_)
                     {
                     ...
                     if(_loc23_)
                     {
                        _loc39_ = _loc24_[0].match(/[A-Za-z]{1,3}/ig);
      ...
      private function initregmap(param1:uint, param2:Boolean) : void
      {
         ...
         REGMAP[FS] = new Register(FS,"texture sampler",5,param2 ? 1024 : 15,REG_FRAG | REG_READ);
         ...
         REGMAP[IID] = new Register(IID,"instance id",7,param2 ? 1024 : 0,REG_VERT | REG_READ);
         REGMAP[VS] = new Register(VS,"vertex texture sampler",5,param2 ? 1024 : 3,REG_VERT | REG_READ);
         REGMAP["op"] = REGMAP[VO];
         REGMAP["i"] = REGMAP[VI];
         REGMAP["v"] = REGMAP[VI];
         REGMAP["oc"] = REGMAP[FO];
